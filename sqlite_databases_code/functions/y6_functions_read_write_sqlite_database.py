...
def drop_table(database,table):
    ...
    if conn:
        ...
        conn.commit()
        print('-- OK DONE : table : functions Deleted ')
def reset_table(database,table):
    ...
    if conn:
        ...
        c=conn.cursor()
        try:
            print("-- try to create : functions in case it does not exit")
            ...
            print("--- OK functions table created")
        except:
            sys.exit("couldn't create functions.db")
        print('- Deleting table : functions =>')
        sql_request="drop table functions"
        c.execute(sql_request)
        conn.commit()
        ...
        try:
            ...
            sql_create=f"CREATE TABLE IF NOT EXISTS functions ( `index` int PRIMARY KEY,name text ,
            c.execute(sql_create)
...
def insert_new_row(row,database,table): # row is a list of items
    ...
    if conn:
        ...
        # STEP 2 : add the new item into the database
        indexA=nb+1
        sqlite_data=(indexA,called_function text ,
            c.execute(sql_create)
            print("--- OK network_objects table created")
        except:
            sys.exit("couldn't create functions.db")
...
def insert_new_row(row,database,table): # row is a list of items
    ...
    if conn:
        ...
        # STEP 2 : add the new item into the database
        indexA=nb+1
        sqlite_data=(indexA,input_variables text ,
            c.execute(sql_create)
            print("--- OK network_objects table created")
        except:
            sys.exit("couldn't create functions.db")
...
def create_database_if_not_exits():
    try:
        ...
        if rep=='Y':
            print('-- Create table : functions')
        create_db_and_table()
        print('-- Ok Done - functions table succesfuly created ')
        rep=input(' Do you want to ingest demo data from ./init/functions.csv ( Y/N ) ? : ')
        if rep=='Y':
            print('-- Ingest demo data into table : functions')
            feed_database()
            print('-- OK  Demo data succesfuly ingested into table : functions')
    except IOError:
