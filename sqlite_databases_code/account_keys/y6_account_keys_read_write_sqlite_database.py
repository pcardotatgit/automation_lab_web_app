...
def delete_from_db(database,table,where_clause):
    ...
    print()
    print('database :',account_keys)     
    print('table :',account_keys)    
...
def drop_table(database,table):
    ...
    if conn:
        ...
        c=conn.cursor()
        print('- Deleting table : account_keys =>')
        sql_request="drop table account_keys"
        c.execute(sql_request)
        conn.commit()
        print('-- OK DONE : table : account_keys Deleted ')
def reset_table(database,table):
    ...
    if conn:
        ...
        c=conn.cursor()
        try:
            print("-- try to create : account_keys in case it does not exit")
            sql_create=f"CREATE TABLE IF NOT EXISTS account_keys ( `index` int PRIMARY KEY,name text ,type text ,username text ,password text ,key text ,comment text)"
            c.execute(sql_create)
            print("--- OK account_keys table created")
        except:
            sys.exit("couldn't create account_keys.db")
        print('- Deleting table : account_keys =>')
        sql_request="drop table account_keys"
        c.execute(sql_request)
        conn.commit()
        ...
        try:
            ...
            sql_create=f"CREATE TABLE IF NOT EXISTS account_keys ( `index` int PRIMARY KEY,name text ,
            c.execute(sql_create)
...
def insert_new_row(row,database,table): # row is a list of items
    ...
    if conn:
        ...
        # STEP 2 : add the new item into the database
        indexA=nb+1
        sqlite_data=(indexA,username text ,
            c.execute(sql_create)
            print("--- OK network_objects table created")
        except:
            sys.exit("couldn't create account_keys.db")
...
def insert_new_row(row,database,table): # row is a list of items
    ...
    if conn:
        ...
        # STEP 2 : add the new item into the database
        indexA=nb+1
        sqlite_data=(indexA,password text ,
            c.execute(sql_create)
            print("--- OK network_objects table created")
        except:
            sys.exit("couldn't create account_keys.db")
...
def create_database_if_not_exits():
    try:
        database = os.getcwd()+'/account_keys.db'
        database=database.replace("\\","/")
        print('database is :',database)
        f = open(database)
        print(green("- OK the database exists",bold=True))
        f.close()
        rep=input(' Do you want to create the account_keys table ( Y/N ) ? : ')
        if rep=='Y':
            print('-- Create table : account_keys')
        create_db_and_table()
        print('-- Ok Done - account_keys table succesfuly created ')
        rep=input(' Do you want to ingest demo data from ./init/account_keys.csv ( Y/N ) ? : ')
        if rep=='Y':
            print('-- Ingest demo data into table : account_keys')
            feed_database()
            print('-- OK  Demo data succesfuly ingested into table : account_keys')
    except IOError:
        print(red("- NOK the database DO NOT exists... let's create it",bold=True))
        print('Create account_keys.db and table : account_keys')
        create_db_and_table()
        print('-- OK  Database and table created')
        print()
        rep=input(' Ingest demo data from ./init/account_keys.csv ( Y/N ) ? : ')
        if rep=='Y':
            print('-- Ingest demo data into table : account_keys')
            feed_database()
            print('-- OK  Demo data ingested into table : account_keys')
        
'''    
if __name__=='__main__':
    database="account_keys.db"
    table="account_keys"
    #main(database,table)   
