#  def_create_route_for_db***
def create_route_for_db(name):
    '''
    MODIFIED : 2025-09-23T09:20:21.000Z

    description : create a new files structure for management of a new database
    
    how to call it : result=create_route_for_db(name)
    '''
    route="/create_route_for_db"
    env.level+='-'
    print('\n'+env.level,white('def create_route_for_db() in app.py  : >\n',bold=True))
    loguer(env.level+' def create_route_for_db() in app.py  : > ')
    # ===================================================================    
    db=name
    ...
    if fichier_route.is_file() or filename in text_content2:
        ...
        portfolio='''
                        ...
                        <div class="col-4 col-6-medium col-12-small">
                            <article class="box style2">
                                <a href="/'''+db+'''_read" class="image featured"><img src="../static/images/database0.png" alt="" /></a>
                                <h3><a href="/'''+db+'''_read">Read Database content</a></h3>
                        ...
                        <div class="col-4 col-6-medium col-12-small">
                            <article class="box style2">
                                <a href="/'''+db+'''_ingest_csv" class="image featured"><img src="../static/images/database0.png" alt="" /></a>
                                <h3><a href="/'''+db+'''_ingest_csv">Ingest a CSV file</a></h3>
                                <p>Ingest a CSV file</p>
                            </article>
                        </div>       
                        <div class="col-4 col-6-medium col-12-small">
                            <article class="box style2">
                                <a href="/'''+db+'''_add_entry" class="image featured"><img src="../static/images/database0.png" alt="" /></a>
                                <h3><a href="/'''+db+'''_add_entry">Add Entry</a></h3>
...
'''            
        menu='''
                    ...
                    <li><a href="javascript:popup_window('/page_info?page=route_def_'''+db+'''_create_db.py&route='''+route+'''','page_info',700,600);">:</a></li>
        '''       
        output='''<!DOCTYPE HTML>
<!-- description-->
<html>
...
'''        
        ...
        result=1
    # ===================================================================
    loguer(env.level+' def END OF create_route_for_db() in app.py  : > ')
    env.level=env.level[:-1]
    return result
